...
export const headerLogoRef = React.createRef<HTMLElement>();
...
const Header = ({ variant }: { variant?: string }) => {
  ...
  const { locationsData, isLocationsLoading } = useSelector((state: RootState) => state.locations);

  const [anchorElUser, setAnchorElUser] = useState<null | HTMLElement>(null);

  const { fetchAllLoading } = useSelector((state: RootState) => state.all);

  const [selectedLocation, setSelectedLocation] = useState<{ label?: string; province?: string; _id?: string } | null>(
    null
  );

  // debounce onChange input
  ...
  return (
    <header
      className={`${styles["main-header"]} ${
        (variant === "profile" && styles["--profile"]) || (variant === "roomsbylocation" && styles["--profile"])
      }`}
    >
      <Container>
        <Box className={`${styles["nav-bar"]}`}>
          ...
          {/* <Box className={`${styles["nav-bar-search"]}`}>
            <div>
              <Button disabled={fetchAllLoading} color="inherit" size="small" variant="text">
            ...
            <Box component={"form"}>
              <Stack direction={"row"} alignItems="center">
                <Autocomplete
                  ...
                  options={locationsData.map(({ _id, name, province, country }) => ({
                  ...
                  renderOption={(props, option) => (
                    <li {...props} key={option._id}>
                      <Box margin={1} sx={{ width: "100%", borderBottom: "1px solid rgba(0, 0, 0, 0.1)" }}>
                        {option.label}
                      </Box>
                    </li>
                  )}
                    ...
                    if (reason === "input") {
                      ...
                      searchTimeoutRef.current = setTimeout(() => {
                        setSelectedLocation({
                          _id: value,
                          province: value,
                          label: value.replace("-", "").split(" ").join(""),
                        });
                      }, 500);
                        ...
                        onKeyDown: (e) => {
                          if (e.code === "enter" && e.currentTarget.value) {
                            setSelectedLocation({
                              ...
                              label: e.currentTarget.value.replace("-", "").split(" ").join(""),
                            });
                          }
                        },
                      }}
          ...
          <Box className={`${styles["nav-bar-auth"]}`}>
            <div className={`${styles["bca-host"]}`}>
              <Button color="inherit" size="small" variant="text">
                {fetchAllLoading ? <Loading width={100} /> : "Become a Host"}
              </Button>
            </div>
            <div className={`${styles["language-icon"]}`}>
              <Button color="inherit" size="small" variant="text">
                {fetchAllLoading ? <Loading width={20} /> : <LanguageIcon fontSize="small" />}
              </Button>
            </div>
            <Box className={`${styles["nav-bar-menu"]}`}>
              <Button
                ...
                sx={{ p: 0, border: "1px solid #dddddd" }}
              >
                <DensityMediumIcon fontSize="small" />
                <Avatar
                  alt=""
                  src={Boolean(user.avatar) ? user.avatar : undefined}
