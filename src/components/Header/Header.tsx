...
const Header = ({ variant }: { variant?: string }) => {
  ...
  const [selectedLocation, setSelectedLocation] = useState<Partial<LocationId> | null>(null);
  ...
  return (
    <header
      className={`${styles["main-header"]} ${
        variant === "profile" || (variant === "roomsbylocation" && styles["--profile"])
      }`}
    >
      <Container>
            ...
            <Box component={"form"}>
              <Stack direction={"row"} alignItems="center">
                <Autocomplete
                  ...
                  onChange={(e: React.SyntheticEvent, value, reason) => {
                    if (reason === "selectOption") {
                      setSelectedLocation({
                        _id: value?._id,
                        province: value?.city.replace("-", "").split(" ").join(""),
                      });
                ...
                <Button
                  ...
                  onClick={() =>
                    selectedLocation?._id && navigate(`/${selectedLocation?.province}`, { state: selectedLocation._id })
                  }
                >
