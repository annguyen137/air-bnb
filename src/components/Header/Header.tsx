...
const Header = ({ variant }: { variant?: string }) => {
  ...
  const { locationsData, isLocationsLoading } = useSelector(
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("resize", closeAutocomplete);
    };
  }, []);
    ...
    <>
      <header
        className={`${styles["main-header"]} ${
          variant === "profile" && styles["--profile"]
        }`}
        ref={headerRef}
      >
        <Container>
          <Box className={`${styles["nav-bar"]}`}>
            <Box
              ...
              ref={headerLogoRef}
              sx={{
                display: variant === "admin" ? "block !important" : "inherit",
              }}
            >
              <Link to="/" className={""}>
                <svg
                  width={variant === "roomsbylocation" ? "30" : "102"}
                  height="32"
                >
                  <path
        ...
        {fetchAllLoading && (
          <Box className={`${styles["loading-effect"]}`}>
            ...
            <CircularProgress
              size={50}
              thickness={7}
              className={`${styles["loading-round"]}`}
            />
          </Box>
        )}
        <CustomDrawer
                    ...
                    navigate(
                      {
                        ...
                        search: createSearchParams({
                          province:
                            selectedLocation.province !== undefined
                              ? selectedLocation.province.replace("-", "")
                              : "",
                        }).toString(),
                      },
                      { state: selectedLocation }
